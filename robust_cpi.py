...
class robust_CPI(BaseEstimator):
    """
        ...
        def _joblib_predict_one_group(imputation_model, X, j):
            ...
            for _ in range(self.n_permutations):
                ...
                for _ in range(self.n_cal):
                    ...
                    if self.score_proba:
                        y_pred_perm += self.estimator.predict_proba(X_perm)/self.n_cal
                    else:
                        y_pred_perm += self.estimator.predict(X_perm)/self.n_cal
    ...
    def score(self, X, y):
        ...
        out_dict["importance"] = np.array(
            [
                (np.mean(out_dict["loss_perm"][j]) - loss_reference)*self.n_cal/(self.n_cal+1)
